...
#if USE_NEW_LOADER == 1
static int zygote_public_libs_fd = -1;
#else
...
static void mount_zygisk() {
    ...
    if (fd >= 0) {
        ...
        if (xmount(zygisk_file.data(), ZYGOTE_PUBLIC_LIBS, nullptr, MS_BIND, nullptr) == 0)
            zygote_public_libs_fd = xopen(ZYGOTE_PUBLIC_LIBS, O_RDONLY | O_CLOEXEC);
        close(fd);
    }
}

#define DO_MOUNT_ZYGISK if (zygote_public_libs_fd == -1) mount_zygisk();

#define DO_UNMOUNT_ZYGISK \
        if (zygote_public_libs_fd >= 0) { \
            umount2(("/proc/self/fd/" + to_string(zygote_public_libs_fd)).data(), MNT_DETACH); \
            close(zygote_public_libs_fd); \
            zygote_public_libs_fd = -1; \
        }
